...
### 动态配置系统 ([config/index.ts](src/config/index.ts))

> **2026-05 新增**：API Key / JWT_SECRET / CLIENT_URL 等敏感配置已从环境变量迁移至数据库 `system_settings` 表，支持后台管理页面动态修改，实时生效，无需重启。环境变量作为兜底：数据库有值用数据库，数据库无值用 `.env`。

**可动态管理的配置项（白名单）：**

| Key | 说明 | 后端读取方式 |
|-----|------|------------|
| `DASHSCOPE_API_KEY` | ModelScope API Key（聊天 / Embedding / Agent / ASR） | `getSetting('DASHSCOPE_API_KEY')` |
| `ARK_API_KEY` | 火山引擎 ARK（图片生成） | `getSetting('ARK_API_KEY')` |
| `TAVILY_API_KEY` | Tavily 联网搜索 | `getSetting('TAVILY_API_KEY')` |
| `EMAIL_USER` | QQ邮箱 SMTP 账号 | `getSetting('EMAIL_USER')` |
| `EMAIL_PASS` | QQ邮箱 SMTP 授权码 | `getSetting('EMAIL_PASS')` |
| `JWT_SECRET` | JWT 签名密钥（getter 动态读取） | `config.jwt.secret`（getter → `getSetting`） |
| `CLIENT_URL` | 前端访问地址 | `getSetting('CLIENT_URL')` |

**核心函数：**

| 函数 | 作用 |
|------|------|
| `initDynamicConfig()` | 启动时从 DB 加载所有配置到内存，失败回退 `.env` |
| `getSetting(key)` | 读取配置值（内存 > 环境变量 > 空串） |
| `updateSetting(key, value)` | 写入 DB + 更新内存，白名单校验 |
| `getMaskedSettings()` | 返回脱敏列表（前4+***+后4），供管理后台展示 |

**静态配置保留不变：** RAG 参数、模型列表、端口、文件大小限制等无需动态修改的配置仍在 `config` 对象中。

### 环境变量 (.env) — 兜底配置

所有 API Key 生产环境建议通过后台管理页面 `/admin/api-keys` 配置（写入数据库）。`.env` 仅作为数据库读取失败时的兜底。
...
# 火山引擎 ARK API Key（可通过管理后台修改）
ARK_API_KEY=
ARK_BASE_URL=https://ark.cn-beijing.volces.com

# 联网搜索 API Key（可通过管理后台修改）
TAVILY_API_KEY=
...
> **说明**：以上所有 Key 类配置均可在管理后台 `/admin/api-keys` 修改，保存后实时生效，无需重启。数据库无值时自动回退读取 `.env`。
...
| email_verified | TINYINT(1) | DEFAULT 0 | 邮箱是否已验证（1=已验证） |
| created_at | TIMESTAMP | DEFAULT CURRENT_TIMESTAMP | 创建时间 |
| updated_at | TIMESTAMP | ON UPDATE CURRENT_TIMESTAMP | 更新时间 |

> **2026-05 变更**：注册流程改为先验证后入库。`email_verified` 默认为 1（只有验证通过才会创建记录）。已移除 `verification_token` 列。
...
> **2026-05 新增** — 存储 API Key 等动态配置，admin 管理页面修改，实时生效。

| 字段名 | 类型 | 约束 | 说明 |
|-------|------|------|------|
| id | INT | PRIMARY KEY, AUTO_INCREMENT | 主键 |
| key_name | VARCHAR(100) | NOT NULL, UNIQUE | 配置键名（DASHSCOPE_API_KEY 等） |
| value | TEXT | NOT NULL | 配置值 |
| description | VARCHAR(255) | DEFAULT '' | 配置说明 |
| updated_at | TIMESTAMP | ON UPDATE CURRENT_TIMESTAMP | 更新时间 |
| created_at | TIMESTAMP | DEFAULT CURRENT_TIMESTAMP | 创建时间 |

#### 验证码临时存储表 (verification_codes)

> **2026-05 新增** — 注册验证码暂存，验证通过后数据移至 users 表并删除。
...
| username | VARCHAR(50) | NOT NULL | 待注册用户名 |
| password | VARCHAR(255) | NOT NULL | 已加密密码 |
| created_at | TIMESTAMP | DEFAULT CURRENT_TIMESTAMP | 创建时间（5分钟过期） |
...
1. 校验参数 + 检查用户名/邮箱唯一性
2. 密码 bcrypt 加密
3. 存入 `verification_codes` 临时表（**不创建 users 记录**）
4. 发送 6 位验证码邮件
5. 邮件发送失败 → 返回 500

**错误情况：**
- 400: 参数缺失/用户名已存在/邮箱已被注册
- 500: 邮件发送失败或服务未配置

#### 1.1.1 邮箱验证（验证通过后才创建用户）

**接口地址:** `POST /api/user/verify-email`
...
**成功响应 (200):**
```json
{ "success": true, "message": "邮箱验证成功，现在可以登录了" }
```

**流程说明：**
1. 查询 `verification_codes` 表匹配 email + code + 5分钟内
2. 验证通过 → 写入 `users` 表（email_verified=1）→ 删除临时记录
3. 验证失败 → 返回 "验证码错误或已过期"

**实现代码位置:** [controllers/user.ts](src/controllers/user.ts)
...
**实现代码位置:** [controllers/user.ts - login()](src/controllers/user.ts)
...
| `fps` | 2（每秒 2 帧） |
| 高性能帧数上限 | 600 帧（默认）/ 300 帧（联网搜索时，`ai.ts` 内自动降档） |
| 低配服务器帧数上限 | 60 帧（`processVideo` 内检测 ≤2核或空闲<1GB 时触发） |
| `maxVideoSize` | 500MB |

**低配服务器自适应（`videoProcessor.ts`）：** 启动时检测 CPU 核数和空闲内存。若 ≤2 核或空闲 <1GB：
- 帧数限制到 60 帧，防止 base64 数组 OOM
- 语音转写自动切换 ModelScope ASR API（`iic/SenseVoiceSmall`），无需本地 Whisper 模型
- FFmpeg 加 `-threads 1` 限制单核，避免占满 CPU 导致 nginx 502

**模型：** `Qwen/Qwen3.5-397B-A17B`（397B MoE，统一多模态架构，原生支持图像/视频帧）

**ASR 模型：** 本地 `Xenova/whisper-small` 优先（~500MB），低配服务器（2核以下需空闲≥2GB / 4核以上需空闲≥1.5GB）自动切换 ModelScope ASR API（`iic/SenseVoiceSmall`），无需本地模型，避免推理阶段 OOM 崩溃。
...
统一走服务端 Edge-TTS（微软 330M 参数神经语音），不再依赖浏览器 SpeechSynthesis。前端通过 `<audio>` 标签播放返回的 MP3，PC/手机全浏览器兼容。

**处理流程：**
```
前端 POST /api/voice/tts { text, voiceId? }
  → ttsService.ts: synthesizeSpeech()
    → 文本清洗（去 Markdown / 去特殊符号）
    → ≤2000 字: 单段合成（spawn python → tts_bridge.py → edge-tts）
    → >2000 字: 分段合成 + ffmpeg-static 拼接
  → 返回 audio/mpeg Buffer
```

**语音列表**（13 种中文女声）：晓晓、晓伊、晓辰、晓涵、晓梦、晓墨、晓秋、晓睿、晓双、晓萱、晓颜、晓悠、晓甄

**关键文件：**

| 文件 | 作用 |
|------|------|
| `scripts/tts_bridge.py` | Python Edge-TTS 桥接脚本，接收文本文件 + voice_id + 输出路径 |
| `services/ttsService.ts` | 文本清洗、长文本分段、`ffmpeg-static` 拼接、自动检测 `python3`/`python` |
| `routes/voice.ts` | 路由层：`POST /api/voice/tts` + `GET /api/voice/voices` |

**依赖：** `pip install edge-tts`（服务器需安装 Python edge-tts 库）

**默认语音：** `zh-CN-XiaoxiaoNeural`（晓晓·温柔亲切），可通过 `voiceId` 参数切换
...
#### 5.0 系统配置管理（2026-05 新增）

**获取配置列表:** `GET /api/admin/settings`
...
```json
{
  ...
  "result": {
    "settings": [
      { "key_name": "DASHSCOPE_API_KEY", "description": "ModelScope API Key", "masked": "ms-2***7ba" },
      { "key_name": "JWT_SECRET", "description": "JWT 签名密钥", "masked": "NEXU***2026" }
    ]
  }
}
```
> **安全设计**：GET 返回脱敏值（前4+***+后4），不暴露真实 Key。

**更新配置:** `PUT /api/admin/settings`

**请求参数：**
```json
{ "key_name": "JWT_SECRET", "value": "新的随机密钥" }
```

**说明：** 白名单校验 key_name，写入 DB + 更新内存，实时生效无需重启。
...
**文档预解析机制**：Agent 模式下，用户上传的文档（PDF/DOCX/TXT/MD）在发送给 Agent 之前由服务端调用 `parseDocument()` 预解析为纯文本，直接拼入用户消息。避免 Agent 使用 MCP `filesystem__read_file` 读取 PDF 二进制导致乱码。

**视频联网搜索**：仅当用户消息含搜索意图关键词（搜索/搜一下/最新/今天/新闻等）时才触发联网搜索，不再强制触发。同时搜索时视频帧数从 600 降至 300，避免 token 超限。
...
- [x] **查询重写** — LLM 改写模糊/指代问题
- [x] **混合检索 (Hybrid Search)** — BM25 + 向量加权融合
- [x] **LLM 重排序 (Rerank)** — LLM 语义级精选
- [x] **Small-to-Big 检索** — 小块匹配 → 大窗口上下文
- [x] **元数据过滤** — 按文件类型/日期/文档ID过滤
- [x] **追问检测** — 自动识别追问，复用缓存
- [x] **联网搜索（WebSearch）** — Tavily + DuckDuckGo 兜底
- [x] **API Key 动态配置** — 数据库存储 + 管理后台可视化管理 + 环境变量兜底
- [x] **JWT_SECRET 动态配置** — getter 实时读取，修改即时生效
- [x] **邮箱验证码系统** — 6位验证码 + 先验证后入库 + 5分钟有效期
- [x] **Whisper 低配 API 兜底** — 低配服务器自动切 ModelScope ASR API
- [x] **TTS 浏览器兼容** — 无中文语音浏览器自动切 Edge-TTS
- [x] AI 模型切换功能
### 功能增强
- [ ] 知识库权限共享（多用户协作）
- [ ] 对话导出功能（PDF/Markdown）
...
Agent 注册了 4 个原生 `fs` 工具，替代 MCP 文件系统。纯 Node.js `fs` 模块，零中间层延迟。

| 工具 | 功能 | 安全约束 |
|------|------|----------|
| `fs_read` | 读文件内容 / 列目录 | 必须在 `workspace` 或 `uploads` 内 |
| `fs_write` | 创建/覆盖文件，自动建父目录 | 必须在 `workspace` 或 `uploads` 内 |
| `fs_delete` | 删除文件或目录（递归） | 必须在 `workspace` 或 `uploads` 内 |
| `fs_list` | 列目录树（支持递归，最多 5 层） | 必须在 `workspace` 或 `uploads` 内 |

**安全模型**：`resolveSafe()` 允许两个沙箱根目录 — `workspaceRoot`（AI 生成文件）和 `uploadsRoot`（用户上传文件）。路径解析后必须位于其中一个根目录内，越界抛错。用户上传文件可通过 `../uploads/文件名` 路径被 AI 读取。

**环境变量**：
```env
WORKSPACE_ROOT=./workspace    # AI 生成文件存储目录
```
> **2026-05 变更**：`resolveSafe()` 从仅允许 `workspace` 扩展为同时允许 `workspace` + `uploads`，解决 AI 工具无法读取用户上传文件的问题。

**实现位置**：`server/src/services/fileSystem.ts`，工具注册于 `agent.ts`

---

## 📄 AI 智能文档生成

AI 自主决定 PPT 每页布局和 Word 文档结构，通过结构化数据调用生成工具。
...
```
GET /api/fs/download?file=演示文稿.pptx
```

AI 生成文件后，回复中自动带 `📥 下载` 链接，前端 Markdown 渲染后点击即下载。

**依赖**：`pptxgenjs`（PPT 生成）、`docx`（Word 生成）
**实现位置**：`server/src/services/documentGenerator.ts`，工具注册于 `agent.ts`

---

*本文档最后更新于 2026-05-07 | v8.2 | uploads目录沙箱放行 + AI工具可读取用户上传文件*
